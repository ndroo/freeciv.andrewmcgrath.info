...
-- Auto-take matching player when an authenticated user connects
function conn_established(conn)
  local username = auth.get_username(conn)
  log.normal(string.format("conn_established: user '%s' connected, attempting auto-take", username))

  -- Give the user enough privileges to take a player
  auth.set_cmdlevel(conn, ALLOW.ctrl)

  -- Query the database to check if this username matches a player
  local table_user = get_option("table_user")
  if dbh then
    local query = string.format([[SELECT name FROM %s WHERE name = '%s']],
                                table_user, dbh:escape(username))
    local res = dbh:execute(query)
    if res then
      local row = res:fetch()
      res:close()
      if row then
        log.normal(string.format("conn_established: found user '%s' in DB, will auto-take", username))
      end
    end
  end
end
